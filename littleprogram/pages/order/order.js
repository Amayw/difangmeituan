...
Page({
  ...
  data: {
    ...
    // 取饭时间
    timeindex1: 0,
    timelist1: ['12：00', '12:20', '12:30', '12:40', '12:50', '13:00', '13:10'],
    //预留电话
    user_phone:13572368453, 
    user_remark: "不要香菜",
    // 捎带地址列表
    addresslist:[
      {
  ...
  // },
  //交互代码
  sendziquorder: function () {
    var that = this;
    console.log(that.data.counter_id)
    wx.request({
      url: 'http://npusmartcanteen.nat300.top/npusmartcanteen/user/order/selfPickupOrder/create',//请求地址路径
      data: { 
        phone:that.data.user_phone,
        counterId: that.data.counter_id,				 //窗口id
        openId: "ew3euwhd7sjw9diwkq" ,//用户的微信openid
        buyerGetTimePeriod: that.data.timelist2[that.data.timeindex1],	 //自取餐时间
        items: that.data.sendcart,
        remark:that.data.user_remark
      },
      method: "post",//请求方式
      header: {//请求头
  ...
  sendshaodaiorder: function () {
    var that = this;
    wx.request({
      url: 'http://npusmartcanteen.nat300.top/npusmartcanteen/user/order/piggybackingOrder/create',//请求地址路径
      data: {
        phone: that.data.user_phone,
        counterId: that.data.counter_id,				 //窗口id
        openId: "ew3euwhd7sjw9diwkq",//用户的微信openid
  ...
  onLoad: function (options) {
    ...
    if(options.cart){
      ...
      for(var i=0;i<list.list.length;i++){
        this.data.a.product_id=list.list[i].product_id;
        this.data.a.product_quantity=list.list[i].product_quantity;
        arr.push(this.data.a)
      }
      console.log(arr)
    }

   console.log(options.counter_id)
    this.setData({
      cart: list,
